...
const damageCalculationDPS = (
  ...
  const carryOverDamageCalcDPShot = (remainingArmor, shouldSetHealth) => {
    const damagePPToArmor =
      damagePerProjectile < 6
        ? roundToX(damagePerProjectile / 2, 2)
        : roundToX(damagePerProjectile - 3, 2);
  ...
  armor <= 0
    ...
    : noRemainingArmorCalc(armor)
    ? carryOverDamageCalcDPShot(armor, true)
    : damagePerProjectile < 6
  ...
  setAbilitySummary(prevAbilitySummary => {
    ...
    const adjustedDamage =
      armor <= 0
        ? 0
        : noRemainingArmorCalc(armor)
        ? carryOverDamageCalcDPShot(armor, false)
        : damagePerProjectile < 6
        ? roundToX(damage / 2, 2)
