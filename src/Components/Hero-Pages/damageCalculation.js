...
const damageCalculation = (abilityName,setAbilitySummary,setArmor,setHealth,armor,health,damage,headShotModifier = 1, discordModifier = 1,...amplifiers) => {
    ...
    damageAmplifier += 1
    //Multiply damageAmplifier by discordModifier and headShotModifier
    ...
    const carryOverDamageCalc =()=> {
       //Calculated remaining damage to be subtracted from health
        const remainingDamage = damage < 6 ? roundToX(armor - damage/2,2) : roundToX(armor - damage + 3,2)
        setArmor(0)
    ...
    :(damage < 6 && armor - damage/2 <= 0) || (armor - damage + 3 < 0)  ? carryOverDamageCalc()
    //if target still has armor, and damage is less than 6, do half original damage and subtract from armor   
    :damage < 6 ? setArmor(prevArmor => roundToX(prevArmor -  damage/2,2) )
